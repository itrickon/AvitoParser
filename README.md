...
### Рекомендуется:  
- Ознакомиться с условиями использования Avito
...
- Не использовать для спама или нарушения правил Avito
...
- Поиск объявлений по прямой ссылке на Avito
...
- Учетная запись Avito (при необходимости)
...
- На рабочем столе появится ярлык **Avito Parser**
- Запустите **Avito Parser**
...
Если возникают ошибки Tesseract:
Проверьте путь к Tesseract в настройках программы

Установите русский язык для Tesseract

Перезапустите программу

Если загрузка остановилась:
Нажмите Enter в консоли (если она открыта)

Используйте горячие клавиши Ctrl+S для остановки

Перезапустите программу

Советы и рекомендации
Для повышения эффективности:
Используйте конкретные ключевые слова - "Ремонт iPhone" лучше чем "Телефоны"

Начинайте с малых объемов - 50-100 объявлений для теста

Делайте перерывы - 5-10 минут между большими партиями

Сохраняйте промежуточные результаты - экспортируйте данные после каждого этапа
...
Проверьте качество изображений в папке debug/

При плохом распознавании - проверьте настройки OCR
...
Проверьте папку docs/ (если есть)
...
# Проверка версий
python --version
pip list | grep -E "(playwright|pandas|pytesseract|Pillow)"

# Проверка Tesseract
tesseract --version
tesseract --list-langs

# Переустановка Playwright
playwright install --force chromium
Часто задаваемые вопросы
В: Нужен ли аккаунт Avito для работы?
О: Для поиска объявлений аккаунт не требуется. Для сбора телефонов может потребоваться авторизация для обхода ограничений.

В: Сколько объявлений можно обработать за раз?
О: Рекомендуется 100-200 объявлений за сеанс. Для больших объемов разбивайте на несколько запусков.

В: Почему не все телефоны распознаются?
О: Распознавание зависит от качества изображения. Некоторые номера могут быть защищены или плохого качества.

В: Как обойти капчу?
О: Программа останавливается при обнаружении капчи. Решите ее вручную в браузере и нажмите "Продолжить".

В: Где хранятся результаты?
О: Основные файлы:

avito_parse_results/ - объявления

phones_output.xlsx - готовые телефоны

avito_phones_playwright/ - промежуточные данные

В: Можно ли использовать на Mac или Linux?
О: Да, но требуется настройка путей к Tesseract и установка зависимостей через pip.
...
Расширенные настройки парсинга
...
Обновляйте зависимости через pip

Проверяйте changelog в папке docs/
